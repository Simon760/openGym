...
import { weekFor, weekOfBlock, activeBlock, upcoming, daysUntil, weekIndexAt } from '../lib/blocks.js'
...
export default function Plan() {
  ...
  const week = weekOfBlock(S, shown)
  ...
  const idx = at ? weekIndexAt(at.block, at.from, todayISO()) : 0
  const letters = 'ABCD'
  ...
  const nextTurn = (() => {
    if (!at || nWeeks < 2 || shown == null || shown === idx) return null
  ...
  return <>
    ...
    <div className="cols"><div>
      ...
      <div className="list" style={{ marginBottom: 4 }}>
        <div className="item" onClick={blocksSheet}>
          ...
          <div className="grow" style={{ minWidth: 0 }}>
            <div className="tt">{at ? at.block.name : t('Simple week')}</div>
            <div className="ss">
      ...
      {soon.length > 0 && <div className="small" style={{ color: 'var(--yellow)', margin: '0 2px 10px', lineHeight: 1.45 }}>
      ...
      {nWeeks > 1 && <>
        ...
        <div className="dim small" style={{ margin: '6px 2px 10px', lineHeight: 1.45 }}>
          {(shown == null ? idx : shown) === idx
            ? t('This is the week running now.')
            : t('Not this week — it comes round {0}.', fmtDate(nextTurn, true))}
        </div>
      </>}
      <div className="list" style={{ display: 'flex', flexDirection: 'column' }}>
        {[1, 2, 3, 4, 5, 6, 0].map(d => {
          const r = S.routines.find(x => x.id === week[d])
          return <div key={d} className="item" onClick={() => dayAssignSheet(d, nWeeks > 1 ? (shown == null ? idx : shown) : null)}>
