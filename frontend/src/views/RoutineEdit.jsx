...
import { glyphPicker, exercisePicker, exConfigSheet, confirmSheet, startFlow } from '../sheets.jsx'
  ...
  return <div className="narrow">
    ...
    {r.ex.length > 0 && (() => {
      ...
      return <div className="card" style={{ marginTop: 12 }}>
        ...
        {/* The map says where; the shares say how much. A session can look balanced on the
            silhouette and still be four fifths one muscle. */}
        <MuscleShare load={load} />
      </div>
    })()}
    ...
    {/* Because a routine is very often written in order to do it now — a second session of
        the day above all, which exists for exactly one afternoon before it is started. */}
    {r.ex.length > 0 && <><div style={{ height: 10 }} />
      <Button icon="play" onClick={() => startFlow(r.id)}>{t('Start {0}', r.name)}</Button></>}
    <div style={{ height: 10 }} />
